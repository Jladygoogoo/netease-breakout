...
def compare_models(dataset_path):
    ...
    def random_forest():
        '''
        返回RandomForestClassifier
        '''
        params = {
            "max_depth": 10,
            "n_estimators": 30  
        }
        model = RandomForestClassifier(max_depth=params["max_depth"], n_estimators=params["n_estimators"], random_state=21)
        return model
    ...
    def svc():
        ...
        model = SVC(gamma=params["gamma"], C=params["C"], random_state=21)
        return model


    def adaboost():
        '''
        返回AdaBoostClassifier
        '''
        params = {
            "learning_rate": 0.1,
            "n_estimators": 50
        }
        model = AdaBoostClassifier(learning_rate=params["learning_rate"],
                        n_estimators=params["n_estimators"], random_state=21)
        return model


    def lgbm_test(X, y, evals_result=None):
        '''
        测试lgbm模型，不来自于sklearn包，需要单独测试
        '''
        ...
        model = lgb.train(params, train_data, valid_sets=[test_data], evals_result=evals_result)
    ...
    def cross_valid():
        models = [random_forest(), svc(), adaboost()]
        for model in models:
...
if __name__ == '__main__':
    dataset_path = "../data/dataset/mla1500_0317_vggish.pkl"
    # dataset_path = "../data/dataset/mla1000_0306.pkl"
