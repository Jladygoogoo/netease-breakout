...
class MyDataste(data.Dataset):
	...
	'''
	def __init__(self, json_path, w2v_path, d2v_path):
		'''
		...
		with open(json_path) as f:
			self.breakouts = json.load(f)
		self.ids = list(range(len(self.breakouts)))
		self.conn = MyConn()
		self.d2v_model = Doc2Vec.load(d2v_path)
		self.w2v_model = Word2Vec.load(w2v_path)

	def query_path(self, tid):
		'''
		从数据库中获取音频和歌词数据路径
		'''
		rawmusic_path, lyrics_path = self.conn.query(targets=["rawmusic_path","lyrics_path"],
											conditions={"track_id":tid})[0]
		return rawmusic_path, lyrics_path
	...
	def __getitem__(self, index):
		...
		必须自定义，使用index获取成对数据
		'''
		breakout = self.breakouts[index]

		track_id = breakout["track_id"]
		beta = breakout["beta"]
		feature_words = breakout["feature_words"]
		rawmusic_path, lyrics_path = self.conn.query(
			targets=["rawmusic_path", "lyrics_path"], conditions={"track_id": track_id}
		)[0]
		# rawmusic_path = rawmusic_path.replace("..", "/Users/inkding/Desktop/netease2")
		...
		return beta, mfcc, lyrics_vec, feature_words
...
def get_loader(json_path, w2v_path, d2v_path, batch_size):
	'''
	为MyDataset返回torch.utils.data.DataLoader对象
	'''
	my_dataset = MyDataste(json_path, w2v_path, d2v_path)
